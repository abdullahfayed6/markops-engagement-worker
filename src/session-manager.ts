...
export class SessionManager {
  ...
  private async open(accountId: string, interactive = false): Promise<ActiveSession> {
  ...
  async start(accountId: string) { const session = await this.open(accountId, true); await session.page.goto('https://www.facebook.com/', { waitUntil: 'domcontentloaded', timeout: 45_000 }); const state = await detectSecurityState(session.page); session.status = state === 'ready' ? 'browser_ready' : 'manual_intervention_required'; return this.response(accountId, session, { loggedIn: state === 'ready' && await this.cookiesLoggedIn(session.context) }); }
  ...
  private async manual(accountId: string, current: ActiveSession, url: string, pending?: () => Promise<WorkerResponse>) { await this.close(current, 'cancelled'); const session = await this.open(accountId, true); await session.page.goto(url, { waitUntil: 'domcontentloaded', timeout: 45_000 }); session.status = 'manual_intervention_required'; session.pending = pending; return session; }
  async validate(accountId: string) { const current = this.sessions.get(this.byAccount.get(accountId) ?? ''); if (current) return this.response(accountId, current, { loggedIn: await this.check(current.page) }); const s = await this.open(accountId); try { await s.page.goto('https://www.facebook.com/', { waitUntil: 'domcontentloaded', timeout: 45_000 }); const loggedIn = await this.check(s.page); if (!loggedIn) { const m = await this.manual(accountId, s, 'https://www.facebook.com/'); return this.response(accountId, m, { loggedIn: false, errorCode: 'MANUAL_INTERVENTION_REQUIRED', errorMessage: 'Login or verification is required.' }); } await this.close(s, 'completed'); return this.response(accountId, undefined, { sessionId: s.sessionId, status: 'completed', loggedIn: true }); } catch (e) { await this.close(s, 'failed'); throw e; } }
  ...
  private async react(page: Page, requested: Reaction) { const selected = page.locator('[aria-pressed="true"]').last(); const previousLabel = (await selected.getAttribute('aria-label').catch(() => null) ?? '').toLowerCase(); const previous = reactions.find((x) => previousLabel.includes(x)) ?? null; if (previous === requested) return { confirmed: true, previous, final: requested }; const like = page.getByRole('button', { name: /^like$/i }).last(); if (requested === 'like') { await like.click({ timeout: 5000 }); } else { await like.hover({ timeout: 5000 }); await page.waitForTimeout(700); const target = page.locator(`[aria-label="${requested[0].toUpperCase()}${requested.slice(1)}"]`).last(); if (await target.count()) await target.click(); else await page.getByText(new RegExp(`^${requested}$`, 'i')).last().click(); } await page.waitForTimeout(900); const state = (await page.locator('[aria-pressed="true"]').last().getAttribute('aria-label').catch(() => null) ?? '').toLowerCase(); const final = reactions.find((x) => state.includes(x)) ?? null; return { confirmed: final === requested, previous, final }; }
  private async comment(page: Page, text: string) { if (!text.trim()) throw new ApiError('INVALID_COMMENT', 'comment must be non-empty.'); const before = await page.locator('body').innerText(); if (before.includes(text)) return { confirmed: true, previous: 'existing_comment', final: 'existing_comment' }; const composer = page.locator('[contenteditable="true"][role="textbox"], textarea').last(); if (!await composer.count()) { await page.getByRole('button', { name: /^comment$/i }).last().click({ timeout: 5000 }); } const activeComposer = page.locator('[contenteditable="true"][role="textbox"], textarea').last(); if (!await activeComposer.count()) return { confirmed: false, previous: null, final: null }; await activeComposer.click(); await activeComposer.fill(text); await activeComposer.press('Enter'); await page.waitForTimeout(1200); const after = await page.locator('body').innerText(); return { confirmed: after.includes(text), previous: null, final: after.includes(text) ? 'comment_present' : null }; }
