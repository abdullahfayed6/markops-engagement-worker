...
import { detectInstagramSecurityState, isInstagramLoggedIn } from './instagram-page.js';
import { ApiError, SessionStatus, WorkerResponse } from './types.js';

/** Instagram supports: like (toggle) and comment. No multi-reaction picker. */
type ActionRequest =
...
export class InstagramSessionManager {
  ...
  constructor(
    ...
    private approvals = new ApprovalStore(`${config.INSTAGRAM_PROFILE_ROOT}/.approvals`)
  ) {}
  ...
  private async open(accountId: string, interactive = false, proxy?: string, cookies?: any[]): Promise<ActiveSession> {
    ...
    try {
      ...
      if (cookies && cookies.length > 0) {
        try {
          await context.addCookies(cookies.map((c) => ({ name: c.name, value: c.value, domain: c.domain || '.instagram.com', path: c.path || '/' })));
        } catch (err) { console.error(`[Instagram] Failed to inject cookies for ${accountId}`, err); }
      }
  ...
  private async cookiesLoggedIn(context: BrowserContext) { const c = await context.cookies('https://www.instagram.com'); return c.some((x) => x.name === 'sessionid' && x.value.length > 0); }
  private async check(page: Page) { const state = await detectInstagramSecurityState(page); return state === 'ready' && await this.cookiesLoggedIn(page.context()); }

  async start(accountId: string, proxy?: string, cookies?: any[]) { const session = await this.open(accountId, true, proxy, cookies); await session.page.goto('https://www.instagram.com/', { waitUntil: 'domcontentloaded', timeout: 45_000 }); const state = await detectInstagramSecurityState(session.page); session.status = state === 'ready' ? 'browser_ready' : 'manual_intervention_required'; return this.response(accountId, session, { loggedIn: state === 'ready' && await this.cookiesLoggedIn(session.context) }); }
  ...
  async continue(accountId: string) { const s = this.requireActive(accountId); if (!await this.check(s.page)) { s.status = 'manual_intervention_required'; return this.response(accountId, s, { errorCode: 'MANUAL_INTERVENTION_REQUIRED', errorMessage: 'Complete Instagram login or verification in the browser, then continue.' }); } if (s.pending) return s.pending(); await this.close(s, 'completed'); return this.response(accountId, undefined, { sessionId: s.sessionId, status: 'completed', loggedIn: true }); }
  ...
  async inspect(accountId: string, postUrl: string) {
    ...
    const security = await detectInstagramSecurityState(s.page);
  ...
  private async perform(accountId: string, input: ActionRequest, record: ApprovalRecord): Promise<WorkerResponse> {
    ...
    try {
      ...
      await this.approvals.complete(record, 'outcome_unknown', result.previous, result.final); s.status = 'outcome_unknown'; return this.response(accountId, s, { errorCode: 'OUTCOME_UNKNOWN', errorMessage: 'The final Instagram state could not be confirmed.', result: { approvalId: input.approvalId, previousState: result.previous, finalState: result.final, diagnostic: result.diagnostic } });
    } catch {
      const sec = await detectInstagramSecurityState(s.page).catch(() => 'ready');
      if (sec !== 'ready') { const m = transient ? await this.manual(accountId, s, input.postUrl, undefined, () => this.perform(accountId, input, record)) : s; m.status = 'manual_intervention_required'; m.pending = () => this.perform(accountId, input, record); await this.approvals.complete(record, 'manual_intervention_required'); return this.response(accountId, m, { errorCode: 'MANUAL_INTERVENTION_REQUIRED', errorMessage: 'Instagram requires manual verification before this action can finish.' }); }
  ...
  private async inspectTargetPost(page: Page): Promise<Record<string, unknown>> {
    ...
    // Instagram post: article element contains the post content
    ...
    const authorCandidates = root.locator('a[role="link"] > span, h2 a').filter({ visible: true });
    if (await authorCandidates.count()) {
      visibleAuthorName = (await authorCandidates.first().innerText()).trim() || null;
    }

    // Caption: look for the post description/caption
    let visiblePostText: string | null = null;
    const captionCandidates = root.locator('h1, [class*="Caption"], span[dir]').filter({ visible: true, hasText: /.{5,}/ });
    ...
    if (!visibleAuthorName && !visiblePostText) throw new ApiError('TARGET_POST_NOT_FOUND', 'Could not identify the Instagram post content.');
    return { visiblePostText, visibleAuthorName };
  }

  // ── Actions ───────────────────────────────────────────────────────────────

  /** Toggle like on an Instagram post. Instagram has a simple heart button (Like/Unlike). */
  private async like(page: Page): Promise<{ confirmed: boolean; previous: string | null; final: string | null; diagnostic: string }> {
    await page.waitForTimeout(1500);

    // Instagram like button is labeled "Like" (unliked) or "Unlike" (liked)
    const likeButtons = page.locator('[aria-label="Like"], [aria-label="Unlike"]').filter({ visible: true });
    if (!await likeButtons.count()) return { confirmed: false, previous: null, final: null, diagnostic: 'like_button_not_found' };

    const btn = likeButtons.first();
    const beforeLabel = await btn.getAttribute('aria-label').catch(() => null);
    const wasLiked = beforeLabel === 'Unlike';
    ...
    const afterLabel = await btn.getAttribute('aria-label').catch(() => null);
    const isNowLiked = afterLabel === 'Unlike';
  ...
  private async comment(page: Page, text: string): Promise<{ confirmed: boolean; previous: string | null; final: string | null; diagnostic: string }> {
    ...
    // Click comment area link if visible (e.g. on feed view)
    const commentLink = page.locator('[aria-label*="Comment"], a[href*="/comments/"]').filter({ visible: true }).first();
    if (await commentLink.count()) { await commentLink.click(); await page.waitForTimeout(800); }

    // Find the textarea / contenteditable comment composer
    const composer = page.locator(
      'textarea[placeholder*="comment" i], textarea[aria-label*="comment" i], [contenteditable="true"][aria-label*="comment" i]'
    ...
    // Try dedicated Post button, otherwise Enter key
    const postBtn = page.locator('[type="submit"]:not([disabled]), button:has-text("Post"), button:has-text("نشر")').filter({ visible: true }).first();
